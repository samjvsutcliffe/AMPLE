...
function [sig,epsE,Dalg] = DPconst(epsTr,mCst)
...
phi=pi/9;                                                                   % friction angle (opening angle of yield surface)
psi=pi/36;                                                                  % dilation angle (set equal to phi for associated flow)
c=0;                                                                        % cohesion
...
if f>tol                                                                    % if yielded
  ...
  if fap<tol                                                                % apex return
    ...
    while (itnum<maxit) && ((norm(b(1:3))>tol) || (abs(b(4))>tolf))         % NR loop 
      ...
      dx=-inv(A)*b; epsE=epsE+dx(1:3); dgam=dgam+dx(4);                     % update unknowns
...
sig=Q.'*[sig; zeros(3,1)]; epsE=Q\[epsE; zeros(3,1)];                       % map stress and elastic strains to 6 component stress space